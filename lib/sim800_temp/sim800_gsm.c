...
static void switch_gsm_state(sim800_t*, sim800GsmState_t);
static void switch_sms_state(sim800_t*, sim800SmsState_t);
static void switch_call_state(sim800_t*, sim800SmsState_t);
static void gsm_init_process(sim800_t*, sim800_Event_t, void*);
static void gsm_state_machine(sim800_t*);

__attribute__((weak)) void on_gsm_ring_callback(sim800_t* p, sim800GsmCall_t* p1) {
    (void)p;
    (void)p1;
}
...
static void
creg_parser(sim800_t* p, const char* str, void* param)
{
...
static void
dtmf_parser(sim800_t* p, const char* str, void* param)
{
    ...
    if (*ptr >= '0' && *ptr <= '9')
    {
        p->Gsm.Dtmf.Code = *ptr - '0';
    }
    else if (*ptr >= 'A' && *ptr <= 'D')
    {
        p->Gsm.Dtmf.Code = *ptr - 'A' + 12;
    }
    else if (*ptr == '*' )
    {
        p->Gsm.Dtmf.Code = SIM800_DTMF_CODE_STAR;
    }
    else if (*ptr == '#' )
    {
        p->Gsm.Dtmf.Code = SIM800_DTMF_CODE_HASH;
    }
    else {
        return; /* ERROR */
    }
    ...
    ptr += 2; /* skip [ <CODE>, ] */
    p->Gsm.Dtmf.duration = sim800_parse_int(&ptr);

    on_gsm_dtmf_callback(p, &p->Gsm.Dtmf);
}
...
static void
no_carrier_parser(sim800_t* p, const char* str, void* param)
{
    debug_printf("< %s > %s\n", __func__, str);
    switch_gsm_state(p, SIM800_GSM_STATE_READY);
    p->Gsm.Call.rings = 0;
...
static void
ring_parser(sim800_t* p, const char* str, void* param)
{
    if (p->Gsm.State != SIM800_GSM_STATE_CALL) {
        memset(phone, 0x00, sizeof(phone));
        switch_gsm_state(p, SIM800_GSM_STATE_CALL);
        debug_printf("< %s > incoming call!\n", __func__);
    }

    if (p->Gsm.Call.hangup) {
        _sim800_cmd_unlock(p);
        ...
        debug_printf("< %s > %s %d\n", __func__, str, p->Gsm.Call.rings);
        p->Gsm.Call.rings += 1;
    }

    on_gsm_ring_callback(p, &p->Gsm.Call);
}
...
static void
clcc_parser(sim800_t* p, const char* str, void* param)
{
...
static void
cops_parser(sim800_t* p, const char* str, void* param)
{
...
static void
cusd_parser(sim800_t* p, const char* str, void* param)
{
 ...
 */
static void
sms_send_prompt_parser(sim800_t* p, const char* str, void* param)
{
 ...
 */
static void
sms_send_result_parser(sim800_t* p, const char* str, void* param)
{
    char res[32];
 ...
 */
static void
switch_gsm_state(sim800_t* p, sim800GsmState_t NewGsmState)
{
    switch (NewGsmState)
    {
    ...
    on_gsm_state_callback(p, p->Gsm.State);
}
 ...
 */
static void
switch_sms_state(sim800_t* p, sim800SmsState_t NewSmsState)
{
    switch(NewSmsState)
    {
        case SIM800_SMS_STATE_IDLE:
            memset(&p->Gsm.Sms, 0x00, sizeof(sim800GsmSms_t));
            break;

        case SIM800_SMS_STATE_TRANSMIT_AWAITING_PROMPT:
            sim800_parser_add(p, SEND_DATA_PROMPT, sms_send_prompt_parser, NULL);
 ...
 */
static void
gsm_init_process(sim800_t* p, sim800_Event_t ev, void* param)
{
    static int stage;
    char str[128];
    ...
    switch (stage) {
        case 0: /* Reset to factory defaults */
            sim800_cmd(p, "AT&F0\n", 1000, gsm_init_process, NULL, SIM800_FLOW_ASYNC);
            break;

        case 1: /* Set RINGs before answer */
            sim800_cmd(p, "ATS0=2\n", 1000, gsm_init_process, NULL, SIM800_FLOW_ASYNC);
            break;

        case 2: /* Enable DTMF Detection (interval = 100, key & time reported ) */
        ...
        case 4: /* Set Local Timestamp mode */
            snprintf(str, sizeof(str), "AT+CLTS=%d\n", 1);
            sim800_cmd(p, str, 1000, gsm_init_process, NULL, SIM800_FLOW_ASYNC);
            break;

        case 5: /* Set GSM network operator numeric format */
            snprintf(str, sizeof(str), "AT+COPS=%d,%d\n", 0,2);
            sim800_cmd(p, str, 1000, gsm_init_process, NULL, SIM800_FLOW_ASYNC);
            break;

        case 6: /* Set SMS magic text format */
            snprintf(str, sizeof(str), "AT+CSMP=17,167,0,25\n"); //,25
            sim800_cmd(p, str, 1000, gsm_init_process, NULL, SIM800_FLOW_ASYNC);
            break;

        case 7: /* Set SMS text mode */
            snprintf(str, sizeof(str), "AT+CMGF=%d\n", 1);
            sim800_cmd(p, str, 1000, gsm_init_process, NULL, SIM800_FLOW_ASYNC);
            break;

        case 8: /* Set SMS text encoding  */
            snprintf(str, sizeof(str), "AT+CSCS=\"%s\"\n", "UCS2");
            sim800_cmd(p, str, 1000, gsm_init_process, NULL, SIM800_FLOW_ASYNC);
            break;

        case 9: /* Disable NetLight LED */
 ...
 */
bool
sim800_call_answer(sim800_t* p)
{
    if (p && p->Gsm.State == SIM800_GSM_STATE_READY)
    {
        return (sim800_cmd(p, "ATA\n", 1000, NULL, NULL,
                           SIM800_FLOW_ASYNC) == SIM800_RESULT_OK);
    }
    return false;
}
 ...
 *
 */
bool sim800_call_hangup(sim800_t* p)
{
    if (p && p->Gsm.State == SIM800_GSM_STATE_CALL)
    {
        p->Gsm.Call.hangup = true;
        return true;
    }
    return false;
}
 ...
 *
 */
bool sim800_ussd_request(sim800_t* p, char* request)
{
    ...
    if (request && p && p->Gsm.State == SIM800_GSM_STATE_READY)
    {
        ptr = &encoded_ussd[0];
        ...
        return (sim800_cmd(p, str, 10 * 1000, NULL, NULL,
                           SIM800_FLOW_ASYNC) == SIM800_RESULT_OK);
    }
    return false;
}
...
bool sim800_sms_send(sim800_t* p, char* phone, char* message)
{
    ...
    if (p && p->Gsm.State == SIM800_GSM_STATE_READY && phone && message)
    {
        ...
        if (sim800_cmd(p, str, 10 * 1000, NULL, NULL,
                       SIM800_FLOW_ASYNC) == SIM800_RESULT_OK)
        {
            ptr = &p->Gsm.Sms.msg[0];
...
on_gsm_state_callback(sim800_t* p, sim800GsmState_t State)
{
    debug_printf("[GSM] ");
    switch (State)
    {
...
__attribute__((weak)) void
on_gsm_call_callback(sim800_t* p, sim800GsmCall_t* pc)
{
    //    uint32_t delta;
    ...
    //            }
    //            else {
    //                printf("'%s', Rings: %d\n", p->Gsm.Call.phone, p->Gsm.Call.rings);
    //            }
    //            break;
    //
    //        case SIM800_GSM_CALL_STATE_DISCONNECT:
    ...
    //            printf("Finished: '%s', time: %d ms\n", p->Gsm.Call.phone, delta);
    //            break;
    //
    //        default:
    //            printf("'%s' state: %d\n", p->Gsm.Call.phone, p->Gsm.Call.State);
    //            break;
    //    }
}

__attribute__((weak)) void
on_gsm_dtmf_callback(sim800_t* p, sim800GsmDtmf_t* pd)
{
...
__attribute__((weak)) void
on_gsm_ussd_callback(sim800_t* p, const char* msg, int format)
{
