...
void wait_for_multiply_sms_response(Sim800SmsSendEvent_t ev) {
    (void) ev;
    Device.State = DEVICE_STATE_SENDING_MULTIPLY_SMS;
}

void wait_for_single_sms_response(Sim800SmsSendEvent_t ev) {
    (void) ev;
...
static void run() {
    switch (Device.State) {
        ...
        case DEVICE_STATE_SENDING_MULTIPLY_SMS:
            ...
            } else if (Sim800.sms_send(Device.smsSender.ptrPhoneNumber, Device.smsSender.txt,
                                       wait_for_multiply_sms_response)) {
        ...
        case DEVICE_STATE_SENDING_SINGLE_SMS:
            if (Sim800.sms_send(Device.smsSender.ptrPhoneNumber, Device.smsSender.txt, wait_for_single_sms_response)) {
                Device.State = DEVICE_STATE_AWAIT_RESPONSE;
            }
            break;
    ...
    switch (Device.userLed.State) {
        case UserLedState_Undefined:
            if (Sim800.State == SIM800_STATE_READY) {
